...
def load_test(img_rows, img_cols):
    ...
    files = glob.glob("/home/r/NerveSegmentation/test/*[0-9].tif")
    X_test = []
    X_test_id = []
    total = 0
    start_time = time.time()
...
def find_best_mask():
    #files = glob.glob(os.path.join('/', 'home', 'r', 'NerveSegmentation', 'train', '*_mask.tif'))
    files = glob.glob("/home/r/NerveSegmentation/train/*_mask.tif")
...
def create_model(img_rows, img_cols):
    model = Sequential()
    model.add(Convolution2D(16, 4, 4, border_mode='same', init='he_normal',
                            input_shape=(1, img_rows, img_cols)))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(0.5))

    model.add(Convolution2D(32, 4, 4, border_mode='same', init='he_normal'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    ...
    sgd = SGD(lr=1e-3, decay=1e-8, momentum=0.98, nesterov=True)
...
def run_cross_validation(nfolds=10):
    ...
    #kf = KFold(len(train_data), n_folds=nfolds, shuffle=False, random_state=None)    
    #print('kf Value: ', kf)
    num_fold = 0
    sum_score = 0
